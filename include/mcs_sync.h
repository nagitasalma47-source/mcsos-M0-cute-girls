...
int mcs_lockdep_before_acquire(
    mcs_lockdep_state_t *state,
    uint32_t class_id,
    const char *name
);

int mcs_lockdep_after_release(
    mcs_lockdep_state_t *state,
    uint32_t class_id,
    const char *name
);

bool mcs_lockdep_is_held(
    const mcs_lockdep_state_t *state,
    uint32_t class_id
);
...
int mcs_mutex_try_lock(
    mcs_mutex_t *mutex,
    uint64_t owner_id
);

int mcs_mutex_unlock(
    mcs_mutex_t *mutex,
    uint64_t owner_id
);
