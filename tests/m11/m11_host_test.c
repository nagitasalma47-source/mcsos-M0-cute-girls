...
static int expect_code(const char *name, int got, int expected) {
    if (got != expected) {
        printf("FAIL %s: got=%s(%d) expected=%s(%d)\n", name,
               m11_error_name(got), got, m11_error_name(expected), expected);
        return 1;
    }
...
int main(void) {
    ...
    failures += expect_code("bad magic",
        m11_elf64_plan_load(image, IMAGE_SIZE, test_region(), &plan), M11_ERR_MAGIC);

    make_valid_image(image);
    ((struct m11_elf64_ehdr *)(void *)image)->e_machine = 3u;
    failures += expect_code("bad machine",
        m11_elf64_plan_load(image, IMAGE_SIZE, test_region(), &plan), M11_ERR_MACHINE);

    make_valid_image(image);
    ((struct m11_elf64_ehdr *)(void *)image)->e_entry = 0x1000u;
    failures += expect_code("entry outside user range",
        m11_elf64_plan_load(image, IMAGE_SIZE, test_region(), &plan), M11_ERR_ENTRY);
    ...
    failures += expect_code("memsz below filesz",
        m11_elf64_plan_load(image, IMAGE_SIZE, test_region(), &plan), M11_ERR_SEGBOUNDS);
    ...
    failures += expect_code("bad alignment",
        m11_elf64_plan_load(image, IMAGE_SIZE, test_region(), &plan), M11_ERR_ALIGN);

    make_valid_image(image);
    ph = (struct m11_elf64_phdr *)(void *)(image + sizeof(struct m11_elf64_ehdr));
    ph[0].p_vaddr = 0x0000800000000000ull;
    failures += expect_code("segment outside user range",
        m11_elf64_plan_load(image, IMAGE_SIZE, test_region(), &plan), M11_ERR_SEGRANGE);
