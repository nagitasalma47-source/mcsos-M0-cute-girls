...
int main(void) {
    ...
    assert(mcsos_syscall_dispatch(
        MCSOS_SYS_PING,0,0,0,0,0,0) == 0x2605020AL);

    assert(mcsos_syscall_dispatch(
        MCSOS_SYS_GET_TICKS,0,0,0,0,0,0) == 12345);
    ...
    assert(mcsos_syscall_dispatch(
        MCSOS_SYS_YIELD,0,0,0,0,0,0) == MCSOS_OK);

    assert(g_yield_count == 1);

    assert(mcsos_syscall_dispatch(
        MCSOS_SYS_EXIT_THREAD,7,0,0,0,0,0) == MCSOS_OK);

    assert(g_exit_code == 7);

    mcsos_syscall_frame_t frame = {
        .nr = MCSOS_SYS_GET_TICKS
    };
