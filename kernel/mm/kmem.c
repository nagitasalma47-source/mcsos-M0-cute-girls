...
static void kmem_coalesce_forward(kmem_block_t *block) {
    while (block != (kmem_block_t *)0 &&
        ...
        expected = (unsigned char *)kmem_align_up_ptr(
            (uintptr_t)expected,
            KMEM_ALIGN
        );
...
int kmem_init(void *base, size_t bytes) {
    if (base == (void *)0 ||
        bytes < (sizeof(kmem_block_t) + KMEM_MIN_SPLIT)) {
        return -1;
    }

    uintptr_t start =
        kmem_align_up_ptr((uintptr_t)base, KMEM_ALIGN);
...
void *kmem_alloc(size_t bytes) {
    ...
    const size_t wanted =
        kmem_align_up_size(bytes, KMEM_ALIGN);

    if (wanted == 0u) {
        return (void *)0;
    }

    for (kmem_block_t *cur = g_head;
         cur != (kmem_block_t *)0;
         cur = cur->next) {
...
int kmem_free_checked(void *ptr) {
    ...
    if (!kmem_ptr_in_heap(block) ||
        block->magic != KMEM_MAGIC) {
        return -3;
    }
...
int kmem_validate(void) {
    if (!g_initialized ||
        g_heap_base == (unsigned char *)0 ||
        g_heap_end <= g_heap_base ||
        g_head == (kmem_block_t *)0) {
        return -1;
    }
    ...
    for (kmem_block_t *cur = g_head;
        ...
        if ((unsigned char *)cur < g_heap_base ||
            ((unsigned char *)cur + sizeof(kmem_block_t)) > g_heap_end) {
            return -5;
        }
        ...
        if (cur->size >
            (size_t)(g_heap_end - kmem_payload(cur))) {
            return -8;
        }
        ...
        cursor = (unsigned char *)
            kmem_align_up_ptr((uintptr_t)cursor, KMEM_ALIGN);
