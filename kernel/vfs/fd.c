...
static mcs_file_t *mcs_fd_get(mcs_fd_table_t *table, int fd) {
    if (!table || fd < 0 || (size_t)fd >= MCS_MAX_OPEN_FILES) return (mcs_file_t *)0;
    if (!table->files[fd].used) return (mcs_file_t *)0;
    return &table->files[fd];
}

static int mcs_fd_alloc(mcs_fd_table_t *table) {
    size_t i;
    if (!table) return MCS_EINVAL;
    for (i = 0; i < MCS_MAX_OPEN_FILES; i++) {
...
int mcs_vfs_open(mcs_fd_table_t *table, mcs_ramfs_t *fs, const char *path, uint32_t flags) {
    mcs_vnode_t *node;
    int fd, rc;
    if (!table || !fs || !path) return MCS_EINVAL;
    if ((flags & (MCS_O_RDONLY | MCS_O_WRONLY | MCS_O_RDWR)) == 0u) flags |= MCS_O_RDONLY;
    rc = mcs_ramfs_lookup(fs, path, &node);
    if (rc != MCS_OK) {
        if ((flags & MCS_O_CREAT) == 0u) return rc;
        rc = mcs_ramfs_create_file(fs, path, &node);
        if (rc != MCS_OK) return rc;
    }
    if (node->type == MCS_VNODE_DIR && mcs_can_write(flags)) return MCS_EISDIR;
...
mcs_ssize_t mcs_vfs_read(mcs_fd_table_t *table, int fd, void *buf, size_t len) {
    mcs_file_t *file;
    size_t remain, n;
    if (!buf && len != 0u) return MCS_EINVAL;
    file = mcs_fd_get(table, fd);
    if (!file) return MCS_EBADF;
    if (!mcs_can_read(file->flags)) return MCS_EACCES;
    if (!file->node || !file->fs) return MCS_EINVAL;
    if (file->node->type == MCS_VNODE_DIR) return MCS_EISDIR;
...
mcs_ssize_t mcs_vfs_write(mcs_fd_table_t *table, int fd, const void *buf, size_t len) {
    ...
    file = mcs_fd_get(table, fd);
    if (!file) return MCS_EBADF;
    if (!mcs_can_write(file->flags)) return MCS_EACCES;
    if (!file->node || !file->fs) return MCS_EINVAL;
    if (file->node->type == MCS_VNODE_DIR) return MCS_EISDIR;
    if ((file->flags & MCS_O_APPEND) != 0u) file->offset = file->node->size;
    if (file->offset > file->node->data_capacity) return MCS_EINVAL;
    n = mcs_min_size(len, file->node->data_capacity - file->offset);
    if (n < len) return MCS_ENOSPC;
...
int mcs_vfs_lseek(mcs_fd_table_t *table, int fd, long offset, int whence) {
    ...
    file = mcs_fd_get(table, fd);
    if (!file) return MCS_EBADF;
    if (!file->node || file->node->type == MCS_VNODE_DIR) return MCS_EISDIR;
    if (whence == MCS_SEEK_SET) base = 0;
    else if (whence == MCS_SEEK_CUR) base = (long)file->offset;
    else if (whence == MCS_SEEK_END) base = (long)file->node->size;
    else return MCS_EINVAL;
    next = base + offset;
    if (next < 0) return MCS_EINVAL;
...
int mcs_vfs_close(mcs_fd_table_t *table, int fd) {
    ...
    if (!file) return MCS_EBADF;
    file->used = 0u; file->flags = 0u; file->offset = 0u;
    file->node = (mcs_vnode_t *)0; file->fs = (mcs_ramfs_t *)0;
    return MCS_OK;
}
...
int mcs_sys_open(mcs_process_t *proc, mcs_ramfs_t *fs, const char *user_path, uint32_t flags) {
    if (!proc || !fs || !user_path) return MCS_EINVAL;
...
mcs_ssize_t mcs_sys_read(mcs_process_t *proc, int fd, void *user_buf, size_t len) {
    if (!proc || (!user_buf && len != 0u)) return MCS_EINVAL;
