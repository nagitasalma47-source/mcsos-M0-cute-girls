...
static void mcs_copy_name(char *dst, const char *src, size_t n) {
    size_t i;
    for (i = 0; i < MCS_MAX_NAME; i++) { dst[i] = '\0'; }
    for (i = 0; i < n && i + 1u < MCS_MAX_NAME; i++) { dst[i] = src[i]; }
}
...
static int mcs_find_child(mcs_ramfs_t *fs, uint32_t parent, const char *name, size_t name_len, mcs_vnode_t **out) {
    ...
    for (i = 0; i < fs->node_count; i++) {
        if (fs->nodes[i].used && fs->nodes[i].parent == parent &&
            mcs_streq_n(name, fs->nodes[i].name, name_len)) {
            *out = &fs->nodes[i];
            return MCS_OK;
        }
    }
...
static int mcs_split_parent_leaf(mcs_ramfs_t *fs, const char *path, mcs_vnode_t **parent, const char **leaf, size_t *leaf_len) {
    ...
    size_t seg_len;
    int rc;
    if (!fs || !path || !parent || !leaf || !leaf_len) return MCS_EINVAL;
    if (path[0] != '/') return MCS_EINVAL;
    if (path[1] == '\0') return MCS_EINVAL;
    if (mcs_strlen(path) >= MCS_MAX_PATH) return MCS_ENAMETOOLONG;
    cur = &fs->nodes[0];
    seg = path + 1;
    for (;;) {
        next = seg;
        ...
        if (seg_len == 0u || seg_len >= MCS_MAX_NAME) return MCS_EINVAL;
        if (*next == '\0') {
            *parent = cur; *leaf = seg; *leaf_len = seg_len;
            return MCS_OK;
        }
        rc = mcs_find_child(fs, cur->id, seg, seg_len, &cur);
        if (rc != MCS_OK) return rc;
        if (cur->type != MCS_VNODE_DIR) return MCS_ENOTDIR;
        seg = next + 1;
    }
}
...
void mcs_ramfs_init(mcs_ramfs_t *fs) {
    ...
    for (i = 0; i < MCS_RAMFS_DATA_BYTES; i++) { fs->data[i] = 0u; }
    fs->node_count = 1u;
    fs->data_used = 0u;
    fs->nodes[0].used = 1u;
    fs->nodes[0].id = 0u;
...
int mcs_ramfs_lookup(mcs_ramfs_t *fs, const char *path, mcs_vnode_t **out_node) {
    ...
    if (path[0] != '/') return MCS_EINVAL;
    if (mcs_strlen(path) >= MCS_MAX_PATH) return MCS_ENAMETOOLONG;
    if (path[1] == '\0') { *out_node = &fs->nodes[0]; return MCS_OK; }
    cur = &fs->nodes[0];
    seg = path + 1;
    while (*seg != '\0') {
        next = seg;
        ...
        rc = mcs_find_child(fs, cur->id, seg, seg_len, &cur);
        if (rc != MCS_OK) return rc;
        if (*next == '\0') { *out_node = cur; return MCS_OK; }
        if (cur->type != MCS_VNODE_DIR) return MCS_ENOTDIR;
        seg = next + 1;
    }
    return MCS_EINVAL;
...
int mcs_ramfs_create_file(mcs_ramfs_t *fs, const char *path, mcs_vnode_t **out_node) {
    ...
    if (rc == MCS_OK) {
        if (existing->type != MCS_VNODE_FILE) return MCS_EISDIR;
        *out_node = existing; return MCS_OK;
    }
    rc = mcs_split_parent_leaf(fs, path, &parent, &leaf, &leaf_len);
    if (rc != MCS_OK) return rc;
    if (parent->type != MCS_VNODE_DIR) return MCS_ENOTDIR;
...
int mcs_ramfs_seed_file(mcs_ramfs_t *fs, const char *path, const uint8_t *data, size_t len) {
    mcs_vnode_t *node;
    int rc;
    if (!fs || !path || (!data && len != 0u)) return MCS_EINVAL;
    rc = mcs_ramfs_create_file(fs, path, &node);
    if (rc != MCS_OK) return rc;
    if (len > node->data_capacity) return MCS_ENOSPC;
