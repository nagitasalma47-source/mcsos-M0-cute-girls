...
int mcsos_user_check_range(uintptr_t addr, size_t len) {
    ...
    if (g_user_region.base == 0u ||
        g_user_region.limit <= g_user_region.base) {
        return 0;
    }
...
int mcsos_copy_from_user(void *dst, const void *src, size_t len) {
    ...
    if (!mcsos_user_check_range((uintptr_t)src, len)) {
        return MCSOS_EFAULT;
    }
...
static int64_t sys_write_serial(uint64_t ptr, uint64_t len, uint64_t a2,
    ...
    if (!mcsos_user_check_range((uintptr_t)ptr, (size_t)len)) {
        return MCSOS_EFAULT;
    }

    return g_ops.write_serial((const char *)(uintptr_t)ptr,
                              (size_t)len);
}
...
void mcsos_syscall_dispatch_frame(mcsos_syscall_frame_t *frame) {
    #ifndef MCSOS_HOST_TEST
    serial_write("[M10] syscall entry reached\n");
#endif
    if (frame == 0) return;

    frame->ret =
        mcsos_syscall_dispatch(frame->nr,
                               frame->arg0,
                               frame->arg1,
                               frame->arg2,
                               frame->arg3,
