...
static mcsos_blk_status_t mcsos_ramblk_rw(
    ...
    if (byte_offset > ram->storage_size ||
        byte_count > ram->storage_size - byte_offset) {
...
mcsos_blk_status_t mcsos_ramblk_init(
    ...
    if (block_size < MCSOS_BLK_DEFAULT_SECTOR_SIZE ||
        !mcsos_is_power_of_two_u32_local(block_size)) {
        return MCSOS_BLK_EINVAL;
    }

    if (storage_size < block_size ||
        (storage_size % block_size) != 0u) {
        return MCSOS_BLK_EINVAL;
    }
